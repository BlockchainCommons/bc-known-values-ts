...
fn run(r: &serde_json::Value) -> String {
    match r["k"].as_str().unwrap() {
        ...
        // The JS-only input domain: nothing to replay.
        "domain" => "js-only".to_string(),
        k => panic!("unhandled {k}"),
    }
}

/// The codepoint a lookup outcome names (`value|name|assignedName`).
fn outcome_codepoint(outcome: &str) -> Option<u64> {
    outcome.split('|').next()?.parse().ok()
}
...
/// only for a codepoint above the constants' range (the bundled vocabularies
/// start at 2000); a wrong name for a constant is a mismatch.
/// D4: `VALUE` (25) and `SELF` (706) are constants the reference declares but
...
fn expected_divergence(r: &serde_json::Value, got: &str, want: &str) -> Option<&'static str> {
    if r["k"] == "lookup" && got == "-" && want != "-" {
        return match outcome_codepoint(want) {
            Some(25) | Some(706) => Some("D4"),
            Some(cp) if cp > 800 => Some("D1"),
            _ => None,
        };
    }
...
/// Pending divergences: port outcomes about to change. Empty for now; a
/// future change adds its classes here and deletes them as they land.
fn pending(_r: &serde_json::Value, _got: &str, _want: &str) -> Option<&'static str> {
    None
}
...
fn main() {
    ...
    for v in &file.vectors {
        ...
        if got == v.expect {
            ok += 1;
        } else if let Some(class) = expected_divergence(&v.recipe, &got, &v.expect).or_else(|| pending(&v.recipe, &got, &v.expect)) {
            expected += 1;
    ...
    println!("{} vectors - {ok} match, {expected} expected-divergence, {js_only} js-only, {mismatch} MISMATCH", file.vectors.len());
